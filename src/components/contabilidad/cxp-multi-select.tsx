    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-background border rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="font-bold text-base">Planilla de pago CxP</h2>
        ...
        <div className="px-5 py-4 space-y-4">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted text-muted-foreground text-xs uppercase tracking-wide">
                  ...
                  <th className="px-3 py-2 text-left">Suplidor</th>
              ...
              <tbody>
                {compras.map((c, i) => (
                  <tr key={c.id} className={i % 2 === 0 ? "bg-background" : "bg-muted/20"}>
                    <td className="px-3 py-2 font-mono text-xs font-medium" style={{ color: "var(--accent-hex)" }}>{c.numero}</td>
                    <td className="px-3 py-2 text-xs">{getSuplidor(c.compraId)}</td>
                    <td className="px-3 py-2 text-right tabular-nums font-medium text-sm">{fmt(c.saldo)}</td>
                  </tr>
                ))}
                <tr className="bg-muted/40 border-t font-bold">
                  <td className="px-3 py-2 text-xs" colSpan={2}>Total a pagar</td>
                  <td className="px-3 py-2 text-right tabular-nums text-base" style={{ color: "var(--accent-hex)" }}>{fmt(total)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="flex gap-2 justify-end pt-1">
            ...
            <button onClick={handleSubmit} disabled={isPending}
              className={cn(
                "px-4 py-2 rounded-lg bg-orange-500 text-white text-sm font-medium hover:bg-orange-600 transition-colors",
...
export function CxPMultiSelect({ grupos }: Props) {
  ...
  const comprasSeleccionadas = grupos.flatMap((g) => g.compras).filter((c) => seleccionadas.has(c.id));
    ...
    <>
      ...
      <div className="space-y-3">
        {grupos.map((g) => {
          ...
          const idsPend     = pendientes.map((c) => c.id);
          const todasSel    = idsPend.length > 0 && idsPend.every((id) => seleccionadas.has(id));
          const algunaSel   = idsPend.some((id) => seleccionadas.has(id));
          ...
          return (
            ...
            >
              <div className="overflow-x-auto">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 border-b text-[10px] font-bold uppercase tracking-wide text-muted-foreground min-w-[860px]">
                  <div className="w-5 shrink-0" />
                  <div className="w-[134px] shrink-0">No. Compra</div>
                  <div className="w-[120px] shrink-0">No. Factura</div>
                ...
                {g.compras.map((c) => (
                  <div key={c.id} className={cn(
                    "px-3 py-2.5 flex items-center gap-2 border-b last:border-0 min-w-[860px]",
                    ...
                    <div className="w-[76px] shrink-0">
                      <span className="text-xs text-muted-foreground whitespace-nowrap">
                        {c.fechaFactura
                          ? new Date(c.fechaFactura).toLocaleDateString("es-DO", { day: "2-digit", month: "2-digit", year: "2-digit" })
                          : "—"}
                      </span>
                    </div>
