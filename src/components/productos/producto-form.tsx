...
export function ProductoForm({ productoId, categorias, defaultValues, nextCodigo }: ProductoFormProps) {
  ...
  // Controla qué campo fue modificado último para el auto-cálculo bidireccional de precios
  ...
  /**
   * Sincronización bidireccional costo ↔ ganancia ↔ precio (solo en creación).
   * - Si el usuario cambió el precio → calcula % ganancia.
   * - Si el usuario cambió ganancia o costo → calcula precio de venta.
   * El ref `lastField` evita bucles infinitos y determina la dirección del cálculo.
   */
  useEffect(() => {
    if (esEdicion || costo <= 0) return;

    if (lastField.current === "precio" && precioVal > 0) {
      // precio → calcular % ganancia
      const nueva = Math.round(((precioVal / costo) - 1) * 10000) / 100;
      if (Math.abs(nueva - ganancia) > 0.005) form.setValue("porcentajeGanancia", nueva);
    } else {
      // ganancia (o costo) → calcular precio
      const nuevo = Math.round(costo * (1 + ganancia / 100) * 100) / 100;
      if (Math.abs(nuevo - precioVal) > 0.005) form.setValue("precioVenta", nuevo);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [costo, ganancia, precioVal]);

  // Detectar duplicados por nombre (debounce 500ms)
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit as any)} className="space-y-6 max-w-4xl">

      {serverError && (
        <div ref={errorRef} className="rounded-md bg-destructive/10 border border-destructive/20 px-4 py-3 text-sm text-destructive font-medium">
          ⚠️ {serverError}
        </div>
      )}
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="codigo" placeholder="PROD-00001" {...form.register("codigo")} />
            {errors.codigo && <p className="text-xs text-destructive">{errors.codigo.message}</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="codigoBarras">Código de barras</Label>
            <Input id="codigoBarras" placeholder="7896543210123" {...form.register("codigoBarras")} />
          </div>
          ...
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <Textarea id="descripcion" rows={2} placeholder="Especificaciones técnicas, presentación..." {...form.register("descripcion")} />
          </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">Fraccionamiento</CardTitle>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" {...form.register("esFraccionable")} className="rounded" />
              <span className="text-sm">Este producto se vende fraccionado</span>
            </label>
          </div>
        </CardHeader>
        {fraccion && (
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input placeholder="PIE, CM, ML..." {...form.register("unidadFraccion")} />
              <p className="text-xs text-muted-foreground">Ej: si la unidad es ROLLO, la fracción sería PIE</p>
            </div>
            <div className="space-y-1.5">
              <Label>Factor de conversión</Label>
              <Input type="number" step="0.0001" placeholder="100" {...form.register("factorFraccion")} />
              <p className="text-xs text-muted-foreground">Cuántas fracciones hay por unidad (ej: 100 pies por rollo)</p>
            </div>
          </CardContent>
        )}
      </Card>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm">Es un <strong>servicio</strong> (precio variable por venta)</span>
            </label>
          </div>
        </CardHeader>
        {form.watch("esServicio") && (
          <div className="px-6 pb-4 text-xs text-muted-foreground">
            Al agregar este producto en Ventas o PDV, cualquier usuario podrá ingresar el precio manualmente.
            Ideal para servicios como <strong>Acarreo</strong>, instalación o mano de obra donde el monto varía según cada cliente.
          </div>
        )}
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="costoUltimo">Costo (RD$)</Label>
            <Input
              id="costoUltimo"
              type="number" step="0.01" min="0" placeholder="0.00"
              {...form.register("costoUltimo")}
            />
          </div>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="precioVenta">Precio de venta (RD$) *</Label>
            <Input
              id="precioVenta"
              type="number" step="0.01" min="0" placeholder="0.00"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="precioMayoreo">Precio al por mayor (RD$)</Label>
            <Input
              id="precioMayoreo"
          ...
          {/* ITBIS — junto a los precios para mayor visibilidad */}
          <div className="sm:col-span-2 pt-1 border-t">
            <label className="flex items-center gap-2 cursor-pointer mt-3">
              <input type="checkbox" {...form.register("exentoItbis")} className="rounded" />
              <span className="text-sm">Este producto está <strong>exento de ITBIS</strong></span>
            </label>
            <p className="text-xs text-muted-foreground mt-1">
              Marcar si no aplica el 18% de ITBIS. Ej: Grava, Arena Itabo, Arena Triturada, Acarreo, Cascajo.
            </p>
          </div>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Control de inventario</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="stockActual" type="number" step="0.0001" min="0" placeholder="0" {...form.register("stockActual")} />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="stockMinimo" type="number" step="0.0001" min="0" placeholder="0" {...form.register("stockMinimo")} />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="stockMaximo">Stock máximo</Label>
            <Input id="stockMaximo" type="number" step="0.0001" min="0" placeholder="Opcional" {...form.register("stockMaximo")} />
          </div>
