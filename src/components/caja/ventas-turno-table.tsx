...
export function VentasTurnoTable({ ventas, modo = "simple" }: Props) {
 ...
 return (
 <tr key={v.id} className="border-b hover:bg-muted/20"> <td className="px-4 py-2 font-mono font-medium"> <div className="flex items-center gap-2"> <Link href={`/ventas/${v.id}`} className="hover:text-primary hover:underline">{v.numero}</Link> <a href={`/caja/factura/${v.id}`} target="_blank" rel="noopener noreferrer" title="Imprimir recibo thermal" className="text-muted-foreground hover:text-foreground transition-colors"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg></a> </div> </td> <td className="px-3 py-2 text-muted-foreground"> {new Date(v.createdAt).toLocaleTimeString("es-DO", { hour: "2-digit", minute: "2-digit" })}
