...
export function VentasTurnoTable({ ventas, modo = "simple" }: Props) {
 ...
 return (
 <tr key={v.id} className="border-b hover:bg-muted/20"> <td className="px-4 py-2 font-mono font-medium"> <Link href={`/ventas/${v.id}`} className="hover:text-primary hover:underline"> {v.numero}
 </Link> </td> <td className="px-3 py-2 text-muted-foreground"> {new Date(v.createdAt).toLocaleTimeString("es-DO", { hour: "2-digit", minute: "2-digit" })}
