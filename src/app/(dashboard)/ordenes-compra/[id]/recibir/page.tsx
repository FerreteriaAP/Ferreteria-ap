...
export default async function RecibirOcPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const items = oc.detalles.map((d) => ({
    ...
    costoUltimo:  Number(d.producto.costoUltimo ?? d.costo),
    precioVenta:  Number(d.producto.precioVenta ?? 0),
    porcentajeGanancia: Number(d.producto.porcentajeGanancia ?? 30),
  }));
  ...
  return (
    <div className="space-y-5">
      <div>
        <Link href={`/ordenes-compra/${id}`} className="text-sm text-muted-foreground hover:text-foreground">
          ← {oc.numero}
        </Link>
        <h1 className="text-2xl font-bold mt-1">Confirmar recepción</h1>
      ...
      <RecepcionOcForm
        ...
        suplidorCredito={oc.suplidor.credito}
        items={items}
      />
    </div>
  );
