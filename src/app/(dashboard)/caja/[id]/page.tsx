...
export default async function TurnoDetallePage({ params }: { params: Promise<{ id: string }> }) {
 ...
 <div className="flex items-center gap-3"> <Link href="/caja" className="text-sm text-muted-foreground hover:text-foreground"> Caja</Link> <span className="text-muted-foreground">/</span> <h1 className="text-xl font-bold">Turno #{turno.numero}</h1> <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-medium ml-1 ${estaAbierto ? "bg-green-100 text-green-700 dark:bg-green-950/40 dark:text-green-400" : "bg-muted text-muted-foreground"}`}> {estaAbierto ? " Abierto" : " Cerrado"}
 </span> </div> {/* Info y resumen */}
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> <div className="rounded-xl border bg-card p-4 space-y-2"> <h2 className="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Información del turno</h2> <dl className="space-y-1 text-sm"> <Row label="Cajero" value={`${turno.usuario.nombre} ${turno.usuario.apellido}`} /> <Row label="Apertura" value={fmtDate(turno.fechaApertura)} /> <Row label="Cierre" value={fmtDate(turno.fechaCierre)} /> <Row label="Monto apertura" value={fmt(turno.montoApertura)} mono /> {turno.montoCierre != null && <Row label="Monto cierre" value={fmt(turno.montoCierre)} mono />}
 ...
 <div className="rounded-xl border bg-card p-4 space-y-2"> <h2 className="font-semibold text-sm text-muted-foreground uppercase tracking-wide">Resumen de cobros</h2> <dl className="space-y-1 text-sm"> <Row label="Total ventas" value={fmt(resumen.totalVentas)} mono /> <Row label="Efectivo" value={fmt(resumen.efectivo)} mono /> <Row label="Tarjeta" value={fmt(resumen.tarjeta)} mono /> <Row label="Transferencia" value={fmt(resumen.transferencia)} mono /> <Row label="Cheque" value={fmt(resumen.cheque)} mono /> {resumen.entradasMov > 0 && <Row label="Entradas manuales" value={fmt(resumen.entradasMov)} mono />}
 {resumen.salidasMov > 0 && <Row label="Salidas manuales" value={fmt(resumen.salidasMov)} mono />}
 <div className="border-t pt-1 mt-1"> <Row label="Efectivo esperado" value={fmt(resumen.montoEsperado)} mono bold /> </div> </dl> </div> )}
 </div> {/* Movimientos */}
 <div className="rounded-xl border bg-card overflow-hidden"> <div className="flex items-center justify-between px-4 py-3 border-b"> <h2 className="font-semibold">Movimientos de caja</h2> {estaAbierto && (
 ...
 <div className="rounded-xl border bg-card overflow-hidden"> <div className="px-4 py-3 border-b"> <h2 className="font-semibold"> Facturas cobradas{" "}
...
function Row({ label, value, mono, bold }: { label: string; value: string; mono?: boolean; bold?: boolean }) {
 return (
 <div className="flex justify-between py-0.5"> <dt className="text-muted-foreground">{label}</dt> <dd className={`${mono ? "font-mono" : ""} ${bold ? "font-bold" : "font-medium"}`}>{value}</dd> </div> );
}
