...
export default async function TurnoDetallePage({ params }: { params: Promise<{ id: string }> }) {
 ...
 <div className="rounded-xl border bg-card p-4 space-y-2"> <h2 className="font-semibold text-sm uppercase tracking-wide" style={{ color: "var(--accent-hex)" }}>Resumen de cobros</h2> <dl className="space-y-1 text-sm"> <Row label="Total ventas" value={fmt(resumen.totalVentas)} mono /> <Row label="Efectivo" value={fmt(resumen.efectivo)} mono /> <Row label="Tarjeta" value={fmt(resumen.tarjeta)} mono /> <Row label="Transferencia" value={fmt(resumen.transferencia)} mono /> <Row label="Cheque" value={fmt(resumen.cheque)} mono /> {resumen.nc > 0 && <Row label="NC aplicadas" value={`−${fmt(resumen.nc)}`} mono />} {resumen.entradasMov > 0 && <Row label="Entradas manuales" value={fmt(resumen.entradasMov)} mono />}
