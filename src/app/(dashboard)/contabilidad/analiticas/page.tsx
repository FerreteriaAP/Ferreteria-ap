...
export default async function AnaliticasPage({ searchParams }: PageProps) {
 ...
 <div className="flex items-center justify-between py-3"> <div> <p className="font-semibold text-sm">Ventas totales</p> <p className="text-xs text-muted-foreground">Subtotal sin ITBIS — el ITBIS no es ingreso del negocio</p> </div> <div className="text-right"> <p className="font-bold text-lg font-mono">{fmt(plVentas, 2)}</p> {varVentas !== null && <Chip v={varVentas} />}
 </div> </div> {/* COGS */}
