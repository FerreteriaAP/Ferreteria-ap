...
export default async function AnaliticasPage({ searchParams }: PageProps) {
 ...
 // URLs para páginas de detalle preservando el período
 const urlTodosClientes  = `/contabilidad/analiticas/clientes?año=${año}&mes=${mes}`;
 const urlTodosProductos = `/contabilidad/analiticas/productos?año=${año}&mes=${mes}`;
 ...
 */}
 ...
 </tbody> <tfoot> <tr className="border-t-2 bg-muted/30"> <td colSpan={3} className="px-5 py-3 font-bold text-sm">Total top 10</td> <td className="px-4 py-3 text-right font-mono font-bold text-xs"> {fmt(porCliente.reduce((s, c) => s + c.totalFacturado, 0), 2)}
 ...
 TOP 10 PRODUCTOS 
 */}
 <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: CARD_BG }}> <div className="px-5 py-3 border-b flex items-center justify-between" style={{ backgroundColor: HEADER_BG }}> <h2 className="font-semibold text-sm">Top 10 productos — {etiquetaPeriodo}</h2> <a href={urlTodosProductos} className="text-xs px-2.5 py-1 rounded-md border font-medium transition-colors hover:opacity-80" style={{ borderColor: "#F47717", color: "#F47717" }}> Ver todos </a> </div> {topProductos.length === 0 ? (
 <p className="text-sm text-muted-foreground py-10 text-center">Sin ventas en el período</p> ) : (
